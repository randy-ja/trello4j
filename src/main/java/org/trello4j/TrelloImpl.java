...
public class TrelloImpl implements Trello {
    ...
    @Override
    public Checklist addChecklistToCard(String cardId, String name) {
        validateObjectId(cardId);
        ...
        final String url = TrelloURL
                .create(apiKey, TrelloURL.CHECKLIST_POST_URL)
                .token(token)
                .build();
        return trelloObjFactory.createObject(new TypeToken<Checklist>() {
        }, doPost(url, map));
    ...
    @Override
    public CheckItem addItemToChecklist(String checklistId, String name) {
        validateObjectId(checklistId);
